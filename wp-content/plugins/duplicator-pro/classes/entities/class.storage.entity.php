...
//
                        ...
                        if ($dropbox_upload_info->error_details == null) {
                            ...
                            if ($dropbox_upload_info->file_meta != null) {
                                ...
                                if (!$this->dropbox_skip_archive_validation_hash && !$is_file_content_hash_property_exists) {
                                    DUP_PRO_LOG::trace('$dropbox_upload_info->file_meta->content_hash property doesn\'t exists in response, so requesting file meta again to the dropbox API server.');
        ...
        if (file_exists($source_archive_filepath)) {
            if (file_exists($source_installer_filepath)) {
                ...
                try {
                    ...
                    // error_log("** Catch Problems copying package $package->Name to $this->onedrive_storage_folder. " . $e->getMessage());
    ...
    private function set_onedrive_archive_offset($upload_info, $onedrive_upload_info = null)
    {
        ...
        if (!empty($upload_info->data)) {
            ...
            DUP_PRO_Log::trace("Calling GET resume URL to get next offset");
            ...
            if (200 == $response_code) {
                ...
                if ($next_expected_range_parts[0] > 0) {
                    ...
                    DUP_PRO_Log::trace("Got OneDrive Archive offset $archive_offset from GET resume URL");
                }
            } 
        } 
                ...
                try {
                    ...
                    if ((!$upload_info->copied_archive) && (!$tried_copying_installer)) {
                        ...
                        $gdrive_upload_info = DUP_PRO_GDrive_U::upload_file_chunk($google_client, $source_archive_filepath, $upload_info->data, $global->gdrive_upload_chunksize_in_kb * 1024,
                                10, $upload_info->archive_offset, $upload_info->upload_id, (50 + $this->throttleDelayInUs));
    ...
    private function set_gdrive_archive_offset($upload_info) {
        ...
        if (is_null($resume_url)) {
            ...
            switch ($response_code) {
                case 308:
                    DUP_PRO_LOG::trace("Google Drive transfer is incomplete");
