...
final class DUPX_Plugins_Manager
{
    ...
    private function __construct()
    {
        ...
        DUPX_Log::info('CONSTRUCT PLUGINS OBJECTS: '.DUPX_log::varToString($this->plugins), DUPX_Log::LV_HARD_DEBUG);
    }
    ...
    private function setCustomPluginsActions()
    {
        ...
        $this->customPluginsActions[self::SLUG_DUPLICATOR_TESTER] = new DUPX_Plugin_custom_actions(self::SLUG_DUPLICATOR_TESTER, DUPX_Plugin_custom_actions::BY_DEFAULT_ENABLED, true, null);

        $longMsg = "This plugin is deactivated by default automatically. <strong>You must reactivate from the WordPress admin panel after completing the installation</strong> "
        ...
        $this->customPluginsActions[self::SLUG_JETPACK]               = new DUPX_Plugin_custom_actions(self::SLUG_JETPACK, DUPX_Plugin_custom_actions::BY_DEFAULT_DISABLED, true, $longMsg);
        ...
        $this->customPluginsActions[self::SLUG_WP_ROCKET] = new DUPX_Plugin_custom_actions(self::SLUG_WP_ROCKET, DUPX_Plugin_custom_actions::BY_DEFAULT_DISABLED, true, $longMsg);

        $longMsg = "This plugin is deactivated by default automatically due to issues that one may encounter when migrating. "
            ."<strong>You must reactivate from the WordPress admin panel after completing the installation</strong> "
            ."or from the plugins tab.";
        $this->customPluginsActions[self::SLUG_WOO_ADMIN] = new DUPX_Plugin_custom_actions(self::SLUG_WOO_ADMIN, DUPX_Plugin_custom_actions::BY_DEFAULT_DISABLED, true, $longMsg);
    }
    ...
    public function pluginExists($slug)
    {
    ...
    public function executeActions($dbh, $subsiteId = -1)
    {
        ...
        if ($this->pluginExists(self::SLUG_DUPLICATOR_TESTER) && !array_key_exists(self::SLUG_DUPLICATOR_TESTER, $activePluginsList)) {
            if ($s3Funcs->newSiteIsMultisite()) {
        ...
        if ($s3Funcs->newSiteIsMultisite()) {
            ...
            $table = $escapedTablePrefix."options";
